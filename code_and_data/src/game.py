...
import typer
from figutils import (DATA_LOC, SCENARIOS_LOC, VALID_DATASETS,
                      VALID_DATASETS_ORDERED, read_and_adjust)
...
BASE = Path(__file__).absolute().parent.parent / "figs" / "ijcai" / "games"
EPS = 1e-9
...
def ficticious_play(
    ...
    print(f"[brown]Starting Fictitious Play[/brown]")
    assert all(
...
def fixation_prob(game_info: GameInfo, n_per_strategy: int = 5, repetitions: int = 100):
    print(f"[yellow]Starting Fixation Prob calculation[/yellow]")
...
def do_moran(
    ...
    print(f"[brown]Starting Moran Process[/brown]")
    n_strategies = len(game_info.row_strategies)
    population = []
    for i in range(n_strategies):
        population += [i] * n_per_strategy
...
def do_egt(
    ...
    print(f"[magenta]Starting Replicator Dynamics calculation[/magenta]")
    assert all(
        a == b for a, b in zip(game_info.row_strategies, game_info.col_strategies)
    )
    ...
    if base:
        ...
        for i, s in enumerate(game_info.row_strategies):
            ...
            if log_scale:
                plt.xscale("log")
        plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=ncol)
        plt.xlabel(f"Time (fraction of simulation time)")
...
def main(
    ...
    if dry_run:
        ...
        exit(0)
    if max_cores < 0:
        print(f"[blue]Running Serially[/blue]")
