...
import typer
from figutils import (DATA_LOC, DIFFICULTY, MECHANISM_TYPE, OPPOSITION,
                      SCENARIOS_LOC, STRATEGY_TYPE, VALID_DATASETS,
...
def process(
    ...
    if verbose:
        print(f"Final records per Mechanism: data")
        print(data.groupby("Mechanism")["Agent"].count().reset_index())
        print(f"Final scores per strategy: agents")
        ...
        try:
            ...
            for folder, o, d, y in zip(
                ...
                if "_base" not in stats:
                    ...
                    agents.loc[
                        (agents.Domain == dname) & (agents.Year == y), OPP
                    ] = oppo
                    if OPP in v:
                        orig_added = True
                        oppositions.append(
